...
. Ταξινόμηση ενεργών οχημάτων ως `AVAILABLE` ή `UNAVAILABLE` βάσει περιόδων μη διαθεσιμότητας.
. Υπολογισμός κόστους ανά όχημα.
. Φιλτράρισμα οχημάτων με συνολικό κόστος `<= 0`.

[plantuml,search-workflow,svg]
....
@startuml
...
if (Minimum duration valid\nfor each season?) then (yes)
  :Classify vehicle availability;
  if (Include unavailable setting enabled?) then (yes)
    :Keep AVAILABLE and UNAVAILABLE vehicles;
  else (no)
    :Keep AVAILABLE vehicles only;
  endif
...
Το `SystemSettings.includeUnavailableVehiclesInSearch` ελέγχει αν τα `UNAVAILABLE` αποτελέσματα παραμένουν στη λίστα. Η effective προεπιλογή είναι `false`.

[options="header",cols="1,2,2"]
|===
|Toggle|Αποτέλεσμα αναζήτησης|Επιτρεπόμενη συνέχεια
|`OFF`|Μόνο `AVAILABLE`, `bookable=true`|`booking/select` και `booking/reserve`
|`ON`|`AVAILABLE` και `UNAVAILABLE`|Τα `UNAVAILABLE`, `bookable=false` οδηγούνται μόνο σε quote/request
|===

Κάθε στοιχείο `fleet.items` περιλαμβάνει:

* `availabilityStatus`: `AVAILABLE` ή `UNAVAILABLE`,
* `bookable`: boolean βοηθητικό πεδίο για ασφαλή δρομολόγηση του client.

Τα `/api/v1/booking/select` και `/api/v1/booking/reserve` παραμένουν authoritative. Απόπειρα χρήσης μη κρατήσιμου οχήματος απορρίπτεται με HTTP `409` και business code `VEHICLE_UNAVAILABLE`.
...
[plantuml,min-duration-gate,svg]
....
@startuml
...
[plantuml,vehicle-availability,svg]
....
@startuml
...
if (Include unavailable setting?) then (yes)
  :Return available and unavailable vehicles;
else (no)
  :Filter out unavailable vehicles;
endif
:Set availabilityStatus and bookable per result;
stop
@enduml
....

[plantuml,pricing-accumulation,svg]
....
@startuml
...
[plantuml,discount-calculation,svg]
....
@startuml
...
[plantuml,extra-hours-handling,svg]
....
@startuml
